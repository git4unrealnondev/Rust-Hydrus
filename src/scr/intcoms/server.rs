...
impl plugin_ipc_interact {
    ...
    pub fn spawn_listener(&mut self, notify: Sender<()>) -> anyhow::Result<()> {
        ...
        for conn in listener.incoming().filter_map(handle_error) {
            ...
            let mut conn = BufReader::new(conn);
            logging::info_log(&"Incoming connection!".to_string());
            ...
            match &pluginComType {
                ...
                types::eComType::BiDirectional => {//Default
    ...
    ///
    fn send_data_request(&self, conn: &mut BufReader<LocalSocketStream>) -> types::SupportedRequests {
        let buffer:&mut [u8; 16] = &mut [b'0'; 16];
        let b_control = types::x_to_bytes(&types::eControlSigs::SEND);
        conn.get_mut()
            .write_all(b_control)
            .context("Socket send failed").unwrap();
        conn.read(buffer).context("plugin failed 2nd step auth").unwrap();
...
impl DbInteract {
    ...
    ///
    fn data_size_to_b<T>(data_object: &T) -> (usize, Vec<u8>){
        let sze = mem::size_of_val(data_object);
        let tmp = data_object;
        let byt = types::x_to_bytes(tmp).to_vec();
        (sze, byt)
    }
    pub fn dbactions_to_function(&mut self, dbaction: types::SupportedDBRequests) -> (usize, Vec<u8>){
        match dbaction {
            types::SupportedDBRequests::db_tag_id_get(id) => {
                let data = self.db_get_file(&id);
                Self::data_size_to_b(&data)
            },
            types::SupportedDBRequests::relationship_get_tagid(id) => {
                let data = self.db_relationship_get_tagid(&id);
                Self::data_size_to_b(&data)
            },
            types::SupportedDBRequests::file_get_id(id) => {
                let data = self.db_get_file(&id);
                Self::data_size_to_b(&data)
            },
        }
