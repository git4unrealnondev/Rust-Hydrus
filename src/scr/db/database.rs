...
PRAGMA journal_mode = WAL;
    ...
    ",
        ...
        let mut conn = main.get_database_connection();
        let tn = transaction_start(&mut conn);
        // Sets default settings for db settings.
        if !first_time_load_flag {
            // Database Doesn't exist
            main.first_db(&tn);
            main.updatedb(&tn);
            main.load_table(&tn, &sharedtypes::LoadDBTable::Settings);
            main.load_caching(&tn);
        } else {
            // Database does exist.
            logging::log(format!(
    ...
    ///
    pub fn load_caching(&mut self, tn: &Transaction<'_>) {
        let temp;
    ...
    ///
    pub fn setup_defaut_misplaced_location(&self, tn: &Transaction<'_>) -> String {
        let loc = self.location_get(tn);
    ...
    ///
    pub fn check_relationship_tag_relations(&mut self, tn: Transaction<'_>) {
        self.load_table(&tn, &sharedtypes::LoadDBTable::Files);
        self.load_table(&tn, &sharedtypes::LoadDBTable::Relationship);
        self.load_table(&tn, &sharedtypes::LoadDBTable::Tags);
        let mut flag = false;
        logging::info_log("Relationship-Tag-Relations checker starting check");

        for fid in self.file_get_list_id(&tn).iter() {
            for tid in self.relationship_get_tagid(&tn, fid).iter() {
                if self.tag_id_get(&tn, tid).is_none() {
                    flag = true;
                    logging::log(format!(
                        "Relationship-Tag-Relations checker found bad tid {tid} relating to fid: {fid} deleting empty relationship"
                    ));
                    self.relationship_remove(&tn, fid, tid);
                }
            }
        }
        ...
        if flag {
            ...
            self.condense_tags(&tn);
            self.vacuum(&tn);
        }
        logging::info_log("Relationship-Tag-Relations checker ending check");
        self.transaction_flush(tn);
    }

    ///
    /// Correct any weird paths existing inside of the db.
    ///
    pub fn check_db_paths(&self, tn: &Transaction<'_>) {
        let db_paths = self.storage_get_all(tn);

        let file_dump = self.setup_defaut_misplaced_location(tn);
        ...
        for db_path in db_paths.iter() {
            for entry in walkdir::WalkDir::new(db_path).into_iter().flatten() {
                ...
                {
                    if self
                        .file_get_hash(tn, &filename.to_string_lossy().to_string())
                        .is_none()
                    {
                        ...
                        .unwrap();
                        if self.file_get_hash(tn, &hash).is_some() {
    ...
    pub fn backup_db(&mut self, tn: Transaction<'_>) {
        use chrono::prelude::*;

        // If we don't have a location set then eazy out
        let dbloc = match self.get_db_loc(&tn) {
            None => {
                return;
            }
    ...
    pub fn get_file_bytes(&self, tn: &Transaction<'_>, file_id: &usize) -> Option<Vec<u8>> {
        let loc = self.get_file(tn, file_id);
        if let Some(loc) = loc {
    ...
    pub fn get_file(&self, tn: &Transaction<'_>, file_id: &usize) -> Option<String> {
        let file = self.file_get_id(tn, file_id);
        if let Some(file_obj) = file {
            ...
            let location = self.storage_get_string(tn, &file.storage_id).unwrap();

            let sup = [location, self.location_get(tn)];
            for each in sup {
                ...
                if let Some(ref ext_str) = self.extension_get_string(tn, &file.ext_id)
    ...
    ///
    pub fn add_dead_url(&mut self, tn: &Transaction<'_>, url: &String) {
        self.add_dead_url_sql(tn, url);
        match self._cache {
            CacheType::Bare => {}
            _ => {
    ...
    ///
    pub fn check_dead_url(&self, tn: &Transaction<'_>, url: &String) -> bool {
        match self._cache {
            CacheType::Bare => self.does_dead_source_exist(tn, url),
            _ => match self._inmemdb.does_dead_source_exist(url) {
                true => true,
                // Need to check if we have a cache miss
                false => self.does_dead_source_exist(tn, url),
            },
        }
    }
    ...
    pub fn jobs_get_isrunning(&self, tn: &Transaction<'_>) -> HashSet<&sharedtypes::DbJobsObj> {
        match self._cache {
            CacheType::Bare => {
                todo!("Bare implementation not implemineted");
            }
    ...
    pub fn db_sanity_check_file(&mut self, tn: &Transaction<'_>) {
        ...
        /*flist.par_iter().for_each(|feach| {
            ...
            if let Some(filestorage_obj) = self.file_get_id(tn, feach) {
                ...
                loop {
                    let location = self.storage_get_string(tn, &fileinfo.storage_id).unwrap();
    ...
    ///
    pub fn storage_get_all(&self, tn: &Transaction<'_>) -> Vec<String> {
        let mut out = Vec::new();
        for id in 1..usize::MAX {
            if let Some(location) = self.storage_get_string(tn, &id) {
                out.push(location);
            } else {
                break;
            }
        ...
        for (cnt, un) in search.searches.into_iter().enumerate() {
            match un {
                ...
                sharedtypes::SearchHolder::And(tag_ids) => {
                    let file_ids = self.relationship_get_fileid_search_sql(tn, &tag_ids);
    ...
    /// Gets a tag by id
    ///
    pub fn tag_id_get(&self, tn: &Transaction<'_>, uid: &usize) -> Option<sharedtypes::DbTagNNS> {
        match self._cache {
            CacheType::Bare => self.tags_get_dbtagnns_sql(tn, uid),
    ...
    ///
    pub fn tags_max_id(&self, tn: &Transaction<'_>) -> usize {
        match self._cache {
            CacheType::Bare => self.tags_max_return_sql(tn),
    ...
    ///
    pub fn tags_get_list_id(&self, tn: &Transaction<'_>) -> HashSet<usize> {
        match self._cache {
            CacheType::Bare => self.tags_get_id_list_sql(tn),
            _ => self._inmemdb.tags_get_list_id(),
        }
    }

    /// returns file id's based on relationships with a tag
    pub fn relationship_get_fileid(&self, tn: &Transaction<'_>, tag: &usize) -> HashSet<usize> {
        match self._cache {
            CacheType::Bare => self.relationship_get_fileid_sql(tn, tag),
            _ => self._inmemdb.relationship_get_fileid(tag),
        }
    }

    pub fn relationship_get_one_fileid(&self, tn: &Transaction<'_>, tag: &usize) -> Option<usize> {
        //self._inmemdb.relationship_get_one_fileid(tag)
        let temp = self.relationship_get_fileid(tn, tag);
        let out = temp.iter().next();
        out.copied()
    }

    /// Returns tagid's based on relationship with a fileid.
    pub fn relationship_get_tagid(&self, tn: &Transaction<'_>, file_id: &usize) -> HashSet<usize> {
        match self._cache {
            CacheType::Bare => self.relationship_get_tagid_sql(tn, file_id),
    ...
    fn vacuum(&mut self, tn: &Transaction<'_>) {
    ...
    fn analyze(&mut self, tn: &Transaction<'_>) {
    ...
    pub fn first_db(&mut self, tn: &Transaction<'_>) {
    ...
    pub fn updatedb(&mut self, tn: &Transaction<'_>) {
        self.setting_add(
            tn,
        ...
        self.create_default_source_url_ns_id(tn);
    }

    ///
    /// Default caching option for the db
    ///
    fn setup_default_cache(&mut self, tn: &Transaction<'_>) {
        self.setting_add(
            tn,
    ...
    ///
    pub fn loaded_scraper_folder(&mut self, tn: &Transaction<'_>) -> PathBuf {
    ...
    ///
    pub fn loaded_plugin_folder(&mut self, tn: &Transaction<'_>) -> PathBuf {
    ...
    ///
    pub fn create_default_source_url_ns_id(&mut self, tn: &Transaction<'_>) -> usize {
        match self.namespace_get(tn, &"source_url".to_string()) {
    ...
    fn alter_table(&mut self, tn: &Transaction<'_>, original_table: &String, new_table: &String) {
        tn.execute(
    ...
    fn check_table_exists(&mut self, tn: &Transaction<'_>, table: String) -> bool {
        let mut out = false;
    ...
    fn db_table_collumn_getnames(&mut self, tn: &Transaction<'_>, table: &String) -> Vec<String> {
    ...
    /// Sets DB Version
    fn db_version_set(&mut self, tn: &Transaction<'_>, version: usize) {
    ...
    fn db_drop_table(&mut self, tn: &Transaction<'_>, table: &String) {
    ...
    pub fn check_version(&mut self, tn: &Transaction<'_>) -> bool {
        ...
        let mut g1 = self.quer_int(tn, query_string.to_string());
        if g1.len() != 1 {
            error!(
        ...
        if self._active_vers != self._vers {
            ...
            self.clear_cache();
            self.load_table(&tn, &sharedtypes::LoadDBTable::Settings);

            if db_vers == 1 {
                panic!("How did you get here vers is 1 did you do something dumb??")
            } else if db_vers == 2 {
                dbg!(self._vers, self._active_vers);
                self.db_update_two_to_three(&tn);
                db_vers += 1;
            } else if db_vers == 3 {
                self.db_update_three_to_four(&tn);
            } else if db_vers == 4 {
                self.db_update_four_to_five(&tn);
            } else if db_vers == 5 {
                self.db_update_five_to_six(&tn);
            } else if db_vers == 6 {
                self.db_update_six_to_seven(&tn);
            } else if db_vers == 7 {
                self.db_update_seven_to_eight(&tn);
            } else if db_vers == 8 {
                self.db_update_eight_to_nine(&tn);
            }
    ...
    pub fn load_table(&mut self, tn: &Transaction<'_>, table: &sharedtypes::LoadDBTable) {
        ...
        if !self._tables_loaded.contains(table) {
            ...
            match &table {
                ...
                sharedtypes::LoadDBTable::Namespace => {
                    self.load_namespace(tn);
                }
                sharedtypes::LoadDBTable::Parents => {
                    self.load_parents(tn);
                }
                sharedtypes::LoadDBTable::Relationship => {
                    self.load_relationships(tn);
                }
                sharedtypes::LoadDBTable::Settings => {
                    self.load_settings(tn);
                }
                sharedtypes::LoadDBTable::Tags => {
                    self.load_tags(tn);
                }
                sharedtypes::LoadDBTable::DeadSourceUrls => {
                    self.load_dead_urls(tn);
                }
                sharedtypes::LoadDBTable::All => {
                    self.load_table(tn, &sharedtypes::LoadDBTable::Tags);
                    self.load_table(tn, &sharedtypes::LoadDBTable::Files);
                    self.load_table(tn, &sharedtypes::LoadDBTable::Jobs);
                    self.load_table(tn, &sharedtypes::LoadDBTable::Namespace);
                    self.load_table(tn, &sharedtypes::LoadDBTable::Parents);
                    self.load_table(tn, &sharedtypes::LoadDBTable::Relationship);
                    self.load_table(tn, &sharedtypes::LoadDBTable::Settings);
                }
            }
    ...
    pub fn file_add_db(&mut self, tn: &Transaction<'_>, file: sharedtypes::DbFileStorage) -> usize {
        match self._cache {
    ...
    fn load_files(&mut self, tn: &Transaction<'_>) {
        if self._cache == CacheType::Bare {
            return;
        }
        self.load_extensions(tn);
    ...
    ///
    pub fn extension_get_id(&self, tn: &Transaction<'_>, ext: &String) -> Option<usize> {
        match self._cache {
    ...
    /// Gets an ID if a extension string exists
    ///
    pub fn extension_get_string(&self, tn: &Transaction<'_>, ext_id: &usize) -> Option<String> {
        match self._cache {
            CacheType::Bare => self.extension_get_string_sql(tn, ext_id),
    ...
    ///
    pub fn extension_put_string(&mut self, tn: &Transaction<'_>, ext: &String) -> usize {
        match self.extension_get_id(tn, ext) {
            Some(id) => id,
    ...
    pub fn extension_load(&mut self, tn: &Transaction<'_>, id: usize, ext: String) {
        match self._cache {
            CacheType::Bare => {
    ...
    fn load_extensions(&mut self, tn: &Transaction<'_>) {
    ...
    /// Same as above
    fn load_namespace(&mut self, tn: &Transaction<'_>) {
        if self._cache == CacheType::Bare {
            return;
        }
        ...
        for ns in nses {
            self.namespace_add_id_exists(tn, ns);
    ...
    ///
    pub fn file_get_hash(&self, tn: &Transaction<'_>, hash: &String) -> Option<usize> {
        match self._cache {
            CacheType::Bare => self.file_get_id_sql(tn, hash),
    ...
    pub fn tag_get_name(
        ...
        self.tag_get_name_tagobject(tn, tagobj)
    }
    ...
    pub fn tag_get_name_tagobject(
        ...
        match self._cache {
            CacheType::Bare => self.tags_get_id_sql(tn, tagobj),
            _ => self._inmemdb.tags_get_id(tagobj).copied(),
        }
    }

    /// db get namespace wrapper
    pub fn namespace_get(&self, tn: &Transaction<'_>, namespace: &String) -> Option<usize> {
        match self._cache {
            CacheType::Bare => self.namespace_get_id_sql(tn, namespace),
    ...
    pub fn namespace_get_string(
        ...
        match self._cache {
            CacheType::Bare => self.namespace_get_namespaceobj_sql(tn, ns_id),
            _ => self._inmemdb.namespace_id_get(ns_id).cloned(),
        }
    }

    /// Adds a file into the db sqlite. Do this first.
    pub fn file_add(&mut self, tn: &Transaction<'_>, file: sharedtypes::DbFileStorage) -> usize {
        match file {
            sharedtypes::DbFileStorage::Exist(ref file_obj) => {
                if self.file_get_hash(tn, &file_obj.hash).is_none() {
                    let id = self.file_add_db(tn, file.clone());
                    self.file_add_sql(
                        tn,
                        &sharedtypes::DbFileStorage::Exist(sharedtypes::DbFileObj {
                            id: file_obj.id,
            ...
            sharedtypes::DbFileStorage::NoIdExist(ref noid_obj) => {
                if self.file_get_hash(tn, &noid_obj.hash).is_none() {
                    let id = self.file_add_db(tn, file.clone());
                    self.file_add_sql(
                        tn,
                    ...
                    self.file_get_hash(tn, &noid_obj.hash).unwrap()
                }
            }
    ...
    pub fn file_get_id(
        ...
        match self._cache {
            CacheType::Bare => self.files_get_id_sql(tn, file_id),
    ...
    pub fn namespace_add(
        ...
        if let Some(id) = self.namespace_get(tn, name) {
            return id;
        }
        ...
        match self._cache {
            CacheType::Bare => {
                ...
                self.namespace_get(tn, name).unwrap()
            }
            _ => self.namespace_add_inmemdb(tn, name.clone(), description.clone()),
        }
    }
    ...
    pub fn namespace_add_id_exists(
        ...
        match self._cache {
            CacheType::Bare => {
                ...
                self.namespace_get(tn, &ns.name).unwrap()
            }
            _ => self.namespace_add_inmemdb(tn, ns.name, ns.description),
        }
    }

    /// Adds namespace into DB. Returns the ID of the namespace.
    fn namespace_add_inmemdb(
        &mut self,
        tn: &Transaction<'_>,
        name: String,
        description: Option<String>,
    ...
    fn parents_get(
        ...
        match self._cache {
            CacheType::Bare => {
                let tagid = self.parents_get_id_list_sql(tn, parent);
    ...
    pub fn parents_add(&mut self, tn: &Transaction<'_>, par: sharedtypes::DbParentsObj) -> usize {
        match self._cache {
            CacheType::Bare => {
                let tagid = self.parents_get_id_list_sql(tn, &par);
    ...
    pub fn parents_rel_get(&self, tn: &Transaction<'_>, relid: &usize) -> HashSet<usize> {
        match self._cache {
            CacheType::Bare => self.parents_tagid_get(tn, relid),
            _ => self._inmemdb.parents_rel_get(relid, None),
        }
    }

    /// Relates the list of tags assoicated with relations
    pub fn parents_tag_get(&self, tn: &Transaction<'_>, tagid: &usize) -> HashSet<usize> {
        match self._cache {
            CacheType::Bare => self.parents_relatetagid_get(tn, tagid),
    ...
    /// Prints db info
    pub fn debugdb(&self, tn: &Transaction<'_>) {
    ...
    pub fn tag_add(
        ...
        let tag_id = match id {
            None => self.tags_max_id(tn),
            Some(id) => id,
        };

        match self._cache {
            CacheType::Bare => {
                let tag = self.tag_get_name(tn, tags.to_string(), namespace);
                match tag {
                    None => {
                        self.tag_add_sql(tn, &tag_id, tags, &namespace);
                        self.tag_get_name(tn, tags.to_string(), namespace).unwrap()
                    }
                    Some(id) => id,
                }
            }
    ...
    fn relationship_add_sql(&mut self, tn: &Transaction<'_>, file: &usize, tag: &usize) {
    ...
    pub fn relationship_add(
        ...
        match self._cache {
            CacheType::Bare => {
                self.relationship_add_sql(tn, &file, &tag);
            }
            _ => {
    ...
    /// Updates job by id
    fn jobs_update_by_id(&mut self, tn: &Transaction<'_>, data: &sharedtypes::DbJobsObj) {
    ...
    pub fn jobs_add(
        ...
        let id = match id {
            None => self.jobs_get_max(tn),
            Some(id) => id,
        };

        for each in self.jobs_get_all(tn) {
            if time == each.1.time
        ...
        self.jobs_update_db(tn, jobs_obj.clone());
    ...
    pub fn jobs_add_new(
        ...
        self.jobs_update_db(tn, dbjobsobj);
    ...
    pub fn get_db_loc(&self, tn: &Transaction<'_>) -> Option<String> {
        self._dbpath.clone()
    }

    /// database searching advanced.
    pub fn db_search_adv(&self, tn: &Transaction<'_>, db_search: sharedtypes::DbSearchQuery) {
        let namespaceone_unwrap = self.search_for_namespace(tn, &db_search.tag_one);
    ...
    fn search_for_namespace(
        &self,
        tn: &Transaction<'_>,
        search: &sharedtypes::DbSearchObject,
    ) -> Option<usize> {
        match &search.namespace {
            None => search.namespace_id,
            Some(id_string) => self.namespace_get(tn, id_string),
        }
    }
    ...
    pub fn del_from_jobs_byid(&mut self, tn: &Transaction<'_>, id: Option<&usize>) {
        if let Some(id) = id {
            self.del_from_jobs_inmemdb(id);
            self.del_from_jobs_table_sql_better(tn, id);
        }
    }
    ...
    pub fn migrate_relationship_tag(
        ...
        match self._cache {
            ...
            _ => {
                let fileids = self.relationship_get_fileid(tn, old_tag_id);
                for file_id in fileids {
                    self.relationship_add(tn, file_id, *new_tag_id, true);
                    self.relationship_remove(tn, &file_id, old_tag_id);
                }
            }
        }
    ...
    ///
    pub fn migrate_tag(&mut self, tn: &Transaction<'_>, old_tag_id: &usize, new_tag_id: &usize) {
        let tag = match self.tag_id_get(tn, old_tag_id) {
            Some(out) => out,
            None => {
                return;
            }
    ...
    pub fn delete_tag_relationship(&mut self, tn: &Transaction<'_>, tag_id: &usize) {
        // self.transaction_flush();
        let relationships = self.relationship_get_fileid(tn, tag_id);
        ...
        for _fileids in relationships.iter() {
            ...
            for file_id in relationships.clone() {
                ...
                self.relationship_remove(tn, &file_id, tag_id);
            }

            // self.tn.lock().unwrap().execute_batch(&sql).unwrap();
    ...
    ///
    fn parents_migration(&mut self, tn: &Transaction<'_>, old_tag_id: &usize, new_tag_id: &usize) {
        // Removes parent by ID and readds it with the new id
        for parent in self.parents_tagid_remove(tn, old_tag_id) {
            logging::log(format!(
    ...
    ///
    pub fn relationship_remove(&mut self, tn: &Transaction<'_>, file_id: &usize, tag_id: &usize) {
    ...
    pub fn tag_remove(&mut self, tn: &Transaction<'_>, id: &usize) {
        for fid in self.relationship_get_fileid(tn, id).iter() {
            self.relationship_remove(tn, fid, id);
        }

        self._inmemdb.tag_remove(id);
        self.delete_tag_sql(tn, id);
        let rel = &self._inmemdb.parents_remove(id);
        for each in rel {
            //println!("Removing Parent: {} {}", each.0, each.1);
            self.delete_parent_sql(tn, &each.0, &each.1);
        }
    }
    ...
    pub fn parents_tagid_remove(
        ...
        match self._cache {
            CacheType::Bare => {
                let out = self.parents_tagid_tag_get(tn, tag_id);
    ...
    pub fn parents_reltagid_remove(
        ...
        match self._cache {
            CacheType::Bare => {
                let out = self.parents_relate_tag_get(tn, reltag);
        ...
        match self._cache {
            CacheType::Bare => {
                if let Some(limit_to) = limit_to {
                    let temp = self.parents_limitto_tag_get(tn, &limit_to);
    ...
    pub fn namespace_delete_id(&mut self, tn: &Transaction<'_>, id: &usize) {
        logging::info_log(format!("Starting deletion work on namespace id: {}", id));

        // self.vacuum(); self.tn.lock().unwrap().execute("create index ffid on
        // Relationship(fileid);", []);
        if self.namespace_get_string(tn, id).is_none() {
            logging::info_log("Stopping because I cannot get ns string.".to_string());
            return;
        }
        let tagids = self.namespace_get_tagids(tn, id);
        for each in tagids.clone().iter() {
            self.tag_remove(tn, each);

            // tag_sql += &format!("DELETE FROM Tags WHERE id = {}; ", each);
            self.delete_tag_relationship(tn, each);
        }

        // elf.tn.lock().unwrap().execute_batch(&tag_sql).unwrap();
        // self.transaction_flush();
        self._inmemdb.namespace_delete(id);
        self.delete_namespace_sql(tn, id);
    }

    /// Retuns namespace id's
    pub fn namespace_keys(&self, tn: &Transaction<'_>) -> Vec<usize> {
        match self._cache {
            CacheType::Bare => self.namespace_keys_sql(tn),
    ...
    ///
    pub fn condense_file_locations(&mut self, tn: &Transaction<'_>) {
        self.load_table(tn, &sharedtypes::LoadDBTable::Files);

        let mut file_id_list: Vec<usize> = self.file_get_list_id(tn).into_iter().collect();
    ...
    fn get_starting_tag_id(&self, tn: &Transaction<'_>) -> usize {
        let mut out = None;

        for id in 0..self.tags_max_id(tn) {
            if self.tag_id_get(tn, &id).is_some() {
                out = Some(id);
                break;
            }
        }
    ...
    /// NOTE Make this an exclusive transaction otherwise we could drop data
    ///
    pub fn condense_tags(&mut self, tn: &Transaction<'_>) {
        self.load_table(tn, &sharedtypes::LoadDBTable::Tags);

        // Stopping automagically updating the db

        let tag_max = self.tags_max_id(tn);

        let mut flag = false;

        logging::info_log("Starting preliminary tags scanning".to_string());
        for id in self.get_starting_tag_id(tn)..tag_max - 1 {
            if self.tag_id_get(tn, &id).is_none() {
                logging::info_log(format!(
    ...
    pub fn condense_namespace(&mut self, tn: &Transaction<'_>) {
        self.load_table(tn, &sharedtypes::LoadDBTable::Namespace);
        self.load_table(tn, &sharedtypes::LoadDBTable::Tags);

        //self.namespace_delete_id(&2);

        for namespace_id in self.namespace_keys(tn) {
            if self.namespace_get_tagids(tn, &namespace_id).is_empty() {
                dbg!(&namespace_id);
            }
        }
    }
    ...
    /// NOTE Make this an exclusive transaction otherwise we could drop data
    pub fn condense_db_all(&mut self, tn: &Transaction<'_>) {
        self.condense_namespace(tn);
        //self.condense_tags();
    ...
    pub fn namespace_get_tagids(&self, tn: &Transaction<'_>, id: &usize) -> HashSet<usize> {
        match self._cache {
            CacheType::Bare => self.namespace_get_tagids_sql(tn, id),
    ...
    pub fn namespace_contains_id(
        ...
        match self._cache {
            CacheType::Bare => self.namespace_contains_id_sql(tn, tag_id, namespace_id),
            _ => self.namespace_get_tagids(tn, namespace_id).contains(tag_id),
        }
    }
    ...
    pub fn drop_recreate_ns(&mut self, id: &usize) {
        ...
        let tida = self.namespace_get_tagids(tn,id);
    ...
    pub fn file_get_list_id(&self, tn: &Transaction<'_>) -> HashSet<usize> {
        match self._cache {
            CacheType::Bare => self.file_get_list_id_sql(tn),
